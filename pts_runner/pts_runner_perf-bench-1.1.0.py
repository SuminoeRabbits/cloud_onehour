#!/usr/bin/env python3
"""
PTS Runner for perf-bench-1.1.0
...
- Software Dependencies:
  * C/C++ Compiler Toolchain
    ...
    * Bison
    * Flex
- Estimated Install Time: 130 Seconds
- Environment Size: 207 MB
- Test Type: OS
- Supported Platforms: Linux

Test Characteristics:
- Multi-threaded: No (selects benchmark scenario, no runtime thread option exposed by profile)
...
class PerfBenchRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        ...
        """
        self.benchmark = "perf-bench-1.1.0"
    ...
    def export_results(self):
        ...
        for num_threads in self.thread_list:
            ...
            # PTS removes dots from directory names, e.g. "perf-bench-1.1.0" becomes "perf-bench-110"
            result_dir_name = result_name.replace('.', '')
            result_dir = pts_results_dir / result_dir_name
...
def main():
    parser = argparse.ArgumentParser(
        description="Linux perf-bench 1.1.0 Runner",
    ...
    runner = PerfBenchRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
