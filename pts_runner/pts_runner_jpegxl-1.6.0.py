...
- Environment Size: ~200 MB plus image samples
- Test Type: Processor
- Supported Platforms: Linux, MacOSX, BSD, Windows

Test Characteristics:
- Upstream PTS profile: pts/jpegxl-1.6.0
- AppVersion: libjxl 0.10.1
- Multi-threaded: Yes (cjxl uses libjxl worker threads)
- Honors CFLAGS/CXXFLAGS: Yes, through CMake/toolchain environment
- Notable Instructions: SIMD paths selected by libjxl/build environment
- THFix_in_compile: false - Thread count NOT fixed at compile time
- THChange_at_runtime: false - PTS profile does not expose a thread option;
                                thread count is controlled via taskset (CPU affinity)

Test Options (6 configurations):
1. JPEG input, quality 80
2. JPEG input, quality 90
3. JPEG input, quality 100
4. PNG input, quality 80
5. PNG input, quality 90
6. PNG input, quality 100

Metric: Megapixels per second (MP/s), higher is better
Input: 6000x4000 pixel JPEG photograph and PNG sample set
...
class JpegxlRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        ...
        # JPEG XL's PTS wrapper runs cjxl directly; thread scaling is done via taskset.
    ...
    def install_benchmark(self):
        """
        Install jpegxl-1.6.0.

        jpegxl does NOT need reinstallation for each thread count.
        Thread count is controlled at runtime via taskset (CPU affinity).

        Build system: CMake + cmake --build.
        THFix_in_compile=false, THChange_at_runtime=false.
        """
        ...
        # JPEG XL uses CMake; NUM_CPU_CORES is consumed by the PTS install.sh
        # and MAKEFLAGS helps any nested make step spawned by dependencies.
    ...
    def run_benchmark(self, num_threads):
        ...
        Thread control for jpegxl is done via taskset (CPU affinity).
        The PTS wrapper runs cjxl directly, so no NUM_CPU_CORES env var is required.
        ...
        # Note: NUM_CPU_CORES is NOT set for run; thread count is controlled via taskset
        ...
        if num_threads >= self.vcpu_count:
            ...
            # cjxl should only see/use the cores exposed by taskset
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            f.write("="*80 + "\n")
            f.write("JPEG XL 1.6.0 Benchmark Summary\n")
            ...
            f.write("="*80 + "\n")
            f.write("Summary Table (MP/s, higher is better)\n")
            f.write("="*80 + "\n")
    ...
    def run(self):
        """Main execution flow."""
        print(f"{'='*80}")
        print("JPEG XL 1.6.0 Benchmark Runner")
        print(f"{'='*80}")
        ...
        print("[INFO] Thread mode: taskset-based (cjxl uses available cores)")
...
def main():
    parser = argparse.ArgumentParser(
        description="JPEG XL 1.6.0 Benchmark Runner",
...
"""
    ...
    runner = JpegxlRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
