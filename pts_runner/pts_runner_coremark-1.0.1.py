...
- Environment Size: 2.0 MB
- Test Type: Processor
- Supported Platforms: Linux, Solaris, MacOSX, BSD, Windows

Test Characteristics:
- Multi-threaded: Yes (parallel execution across cores)
...
- THChange_at_runtime: false - Requires recompilation to change thread count
"""
...
class PreSeedDownloader:
    ...
    def download_from_xml(self, benchmark_name, threshold_mb=96):
        ...
        if not xml_path.exists():
            ...
            if not xml_path.exists():
                print(f"  [WARN] downloads.xml still missing after info: {xml_path}")
                return
...
class CoreMarkRunner:
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            f.write("="*80 + "\n")
            f.write("CoreMark 1.0.1 Benchmark Summary\n")
    ...
    def run(self):
        """Main execution flow."""
        print(f"{'='*80}")
        print("CoreMark 1.0.1 Benchmark Runner")
        print(f"{'='*80}")
        ...
        print("[INFO] Thread mode: Compile-time fixed (THFix_in_compile=true)")
...
def main():
    ...
    runner = CoreMarkRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
