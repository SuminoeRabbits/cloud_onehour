#!/usr/bin/env python3
"""
PTS Runner for build-llvm-1.6.0

System Dependencies (from phoronix-test-suite info):
- Software Dependencies:
  * C/C++ Compiler Toolchain (gcc-14/g++-14 recommended)
  * CMake
  * Ninja (optional but recommended for speed) or Make
- Estimated Install Time: 60 Minutes
- Environment Size: 30 GB (Source + Build Artifacts)
...
- THFix_in_compile: false - Thread count NOT fixed at compile time (controlled via --jobs)
- THChange_at_runtime: true - Can change threads at runtime via cmake --build . -- -j $NUM_CPU_CORES
"""
...
class BuildLLVMRunner:
    ...
    def install_benchmark(self):
        """
        Install build-llvm-1.6.0 with GCC-14 native compilation.

        Note: Unlike coremark, build-llvm does NOT need reinstallation for each thread count
        because it supports runtime thread configuration via cmake --build . -- -j argument.
        ...
        install_cmd = f'{builder_env_str}LLVM_CMAKE_EXTRA_OPTS="{llvm_cmake_opts}" MAKEFLAGS="-j{nproc}" CC={cc} CXX={cxx} CFLAGS="-O3 -march=native -mtune=native" CXXFLAGS="-O3 -march=native -mtune=native" phoronix-test-suite batch-install {self.benchmark_full}'
