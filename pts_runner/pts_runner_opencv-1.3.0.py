        ...
        if install_sh.exists():
            try:
                ...
                testdata_supplement = (
                    ...
                    f"# dog416.png was added to DNN perf tests after 4.7.0; create a synthetic\n"
                    f"# 416x416 RGB image using only Python stdlib (performance test: speed only).\n"
                    f"python3 - <<'_PYEOF_'\n"
                    f"import os, zlib, struct\n"
                    f"dst = 'opencv_extra-{ver}/testdata/dnn/dog416.png'\n"
                    f"if not os.path.exists(dst):\n"
                    f"    W, H = 416, 416\n"
                    f"    raw = bytearray()\n"
                    f"    for y in range(H):\n"
                    f"        raw.append(0)\n"
                    f"        for x in range(W):\n"
                    f"            raw += bytes([int(y*255/H), int(x*255/W), 128])\n"
                    f"    def chunk(t, d):\n"
                    f"        return struct.pack('>I', len(d)) + t + d + struct.pack('>I', zlib.crc32(t+d) & 0xffffffff)\n"
                    f"    with open(dst, 'wb') as f:\n"
                    f"        f.write(b'\\x89PNG\\r\\n\\x1a\\n')\n"
                    f"        f.write(chunk(b'IHDR', struct.pack('>IIBBBBB', W, H, 8, 2, 0, 0, 0)))\n"
                    f"        f.write(chunk(b'IDAT', zlib.compress(bytes(raw))))\n"
                    f"        f.write(chunk(b'IEND', b''))\n"
                    f"    print('[PATCH] Created synthetic dog416.png')\n"
                    f"_PYEOF_"
                )
