...
class OpenCVRunner:
    ...
    def patch_test_definition(self, keep_tests=None) -> bool:
        ...
        of the upstream entries (e.g. only 'dnn').  This method:
          1. Adds any keep_tests entries that are missing, using _ALL_KNOWN_TESTS
             as the authoritative name->value mapping.
          2. Removes any entries that are NOT in keep_tests.
        """
        keep_tests = set(keep_tests or self._KEEP_TESTS)
        ...
        try:
            ...
            for option in test_settings.findall("Option"):
                ...
                # --- Step 2: add missing keep_tests entries ---
                for val in sorted(keep_tests):
                ...
                # --- Step 3: remove entries not in keep_tests ---
                for entry in menu.findall("Entry"):
                    val_node = entry.find("Value")
                    val = (val_node.text or "").strip() if val_node is not None else ""
                    if val not in keep_tests:
                        menu.remove(entry)
                        patched = True
            ...
            if patched:
                ...
                print(f"  [INFO] Patched test-definition.xml to keep only tests: {keep_tests}")
            else:
                print(f"  [INFO] test-definition.xml already matches keep_tests: {keep_tests}")
            return True
        except Exception as e:
    ...
    def patch_test_definition_app_version(self) -> bool:
        """Update the displayed OpenCV version in test-definition.xml."""
        xml_path = Path.home() / ".phoronix-test-suite" / "test-profiles" / "pts" / self.benchmark / "test-definition.xml"
        if not xml_path.exists():
            return False
        ...
        try:
            tree = ET.parse(xml_path)
            root = tree.getroot()
            app_version = root.find("./TestInformation/AppVersion")
            if app_version is None:
                return False
            if (app_version.text or "").strip() == self._OPENCV_TARGET_VERSION:
                return True
            app_version.text = self._OPENCV_TARGET_VERSION
            tree.write(xml_path, encoding="utf-8", xml_declaration=True)
            print(f"  [PATCH] test-definition.xml: AppVersion -> {self._OPENCV_TARGET_VERSION}")
            return True
        except Exception as e:
            print(f"  [WARN] Failed to patch AppVersion: {e}")
            return False

    # ---------------------------------------------------------------------------
    # OpenCV version patch: override PTS profile to use 5.0.0 instead of 4.7.0
    # ---------------------------------------------------------------------------
    _OPENCV_TARGET_VERSION = "5.0.0"
    _OPENCV_PACKAGES = {
        "opencv": {
    ...
    # Modern opencv_extra archives do NOT ship testdata/dnn/ — those files are stored
    ...
    @staticmethod
    def host_cpu_features() -> set:
        """Return CPU feature flags from /proc/cpuinfo."""
        features = set()
        try:
    ...
    @classmethod
    def host_supports_avx512_skx(cls) -> bool:
        """Return True when the host exposes the AVX-512 feature set used by AVX512_SKX."""
        flags = cls.host_cpu_features()
        required = {"avx512f", "avx512cd", "avx512vl", "avx512bw", "avx512dq"}
        return required.issubset(flags)

    @classmethod
    def host_supports_sve(cls) -> bool:
        """Return True when the host exposes Arm SVE or SVE2."""
        features = cls.host_cpu_features()
        return "sve" in features or "sve2" in features

    def enable_sve_dispatch(self) -> bool:
        """Auto-enable OpenCV's SVE dispatch on capable Arm hosts, with env override."""
        override = os.environ.get("OPENCV_ENABLE_SVE", "auto").strip().lower()
        if override in {"1", "true", "yes", "on", "force"}:
            return True
        if override in {"0", "false", "no", "off"}:
            return False
        return self.host_supports_sve()

    def enable_avx512_dispatch(self) -> bool:
        """Enable AVX512_SKX only when explicitly requested.

        OpenCV 5.0.0 currently fails to compile some imgproc AVX-512 dispatch
        paths on capable x86_64 hosts (warp_common.vector.hpp mixes 256-bit
        and 512-bit SIMD vector types). Keep the default build stable and let
        OPENCV_ENABLE_AVX512=1 opt into the experimental path.
        """
        override = os.environ.get("OPENCV_ENABLE_AVX512", "auto").strip().lower()
        if override in {"1", "true", "yes", "on", "force"}:
            return True
        if override in {"0", "false", "no", "off"}:
            return False
    ...
    def patch_opencv_version(self) -> bool:
        """Patch PTS opencv profile to use OpenCV 5.0.0.
        ...
        try:
            ...
            for pkg in root.findall(".//Package"):
                ...
                # value computed on the developer's machine.  Drop checksum/size
                # metadata and let PTS download the tag archive as served.
                for optional_node in (md5_node, sha256_node, size_node):
                    if optional_node is not None:
                        pkg.remove(optional_node)

            # Add supplemental opencv_extra-4.7.0 for full testdata/ (moved to LFS in newer releases)
        ...
        if install_sh.exists():
            try:
                ...
                # Step 1: replace old version strings with target OpenCV version
                ...
                # `tar -xf opencv_extra-<target>.tar.gz`
                #
                # Modern opencv_extra archives ship only module source code; all testdata
                ...
                # it into the target version's directory so all perf tests can find their assets:
                ...
                testdata_supplement = (
                    ...
                    "# Supplement missing testdata/ from opencv_extra-4.7.0\n"
                    "# (testdata moved to Git LFS in newer releases; absent from source archive)\n"
                    "tar -xf opencv_extra-4.7.0.tar.gz opencv_extra-4.7.0/testdata/\n"
                    f"cp -r opencv_extra-4.7.0/testdata/. opencv_extra-{ver}/testdata/\n"
                    "rm -rf opencv_extra-4.7.0\n"
                    "# dog416.png was added to DNN perf tests after 4.7.0; create a synthetic\n"
                    "# 416x416 RGB image using only Python stdlib (performance test: speed only).\n"
                    "python3 - <<'_PYEOF_'\n"
                    "import os, zlib, struct\n"
                    f"dst = 'opencv_extra-{ver}/testdata/dnn/dog416.png'\n"
                    ...
                    "        raw.append(0)\n"
                    "        for x in range(W):\n"
                    "            raw += bytes([int(y*255/H), int(x*255/W), 128])\n"
                    "    def chunk(t, d):\n"
                    ...
                    "    print('[PATCH] Created synthetic dog416.png')\n"
                    "_PYEOF_"
                )
                if extra_extract_marker in content:
                    content = content.replace(extra_extract_marker, testdata_supplement)
                    print("  [PATCH] install.sh: full testdata supplement injected")
                else:
                    print(f"  [WARN]  install.sh: marker '{extra_extract_marker}' not found; "
                          f"testdata supplement NOT injected")

                # Step 3: replace cmake command with conservative optimized flags.
                # OpenCV 5.0.0 + AVX-512 can fail in imgproc/warp_common.vector.hpp
                # on some x86_64 hosts due to 256-bit/512-bit SIMD type mismatches.
                # Keep x86 explicit and stable: AVX/AVX2 by default, and also pass
                # -mno-avx512* so compiler-native flags cannot pull simd512 into the
                # baseline namespace. AVX512_SKX remains available only via
                # OPENCV_ENABLE_AVX512=1 for experiments.
                # On AArch64, OpenCV exposes SVE (not SVE2) as the CMake dispatch
                # feature. SVE2-capable hosts also report SVE, and KleidiCV can use
                # Arm-specific SVE/SVE2 code paths when enabled.
                import platform
                ...
                if is_arm64:
                    dispatch_flags = ["NEON_FP16", "NEON_BF16", "NEON_DOTPROD"]
                    if self.enable_sve_dispatch():
                        dispatch_flags.append("SVE")
                    dispatch_value = ",".join(dispatch_flags)
                    cmake_new = (
                        "cmake -DCMAKE_BUILD_TYPE=Release -DWITH_OPENCL=OFF"
                        " -DOPENCV_GENERATE_SETUPVARS=ON"
                        f" -DCPU_DISPATCH={dispatch_value}"
                        " -DWITH_KLEIDICV=ON"
                        " -DWITH_IPP=OFF"
                        " .."
                    )
                else:
                    dispatch_flags = ["AVX", "AVX2"]
                    avx512_enabled = self.enable_avx512_dispatch()
                    if avx512_enabled:
                        dispatch_flags.append("AVX512_SKX")
                    dispatch_value = ",".join(dispatch_flags)
                    no_avx512_flags = (
                        "" if avx512_enabled else
                        " -DCMAKE_C_FLAGS=\"-mno-avx512f -mno-avx512cd -mno-avx512bw -mno-avx512dq -mno-avx512vl\""
                        " -DCMAKE_CXX_FLAGS=\"-mno-avx512f -mno-avx512cd -mno-avx512bw -mno-avx512dq -mno-avx512vl\""
                    )
                    x86_simd_note = " AVX512=enabled" if avx512_enabled else " AVX512=disabled"
                    cmake_new = (
                        "cmake -DCMAKE_BUILD_TYPE=Release -DWITH_OPENCL=OFF"
                ...
                cmake_pattern = r'^cmake\s+-DCMAKE_BUILD_TYPE=Release\b.*\.\.\s*$'
                updated_content, cmake_replacements = re.subn(
                    cmake_pattern,
                    cmake_new,
                    content,
                    count=1,
                ...
                if cmake_replacements:
                    content = updated_content
                    print(f"  [PATCH] install.sh: cmake SIMD flags injected [{arch_label}]"
                          + (f" dispatch={dispatch_value} +KLEIDICV" if is_arm64 else f" dispatch={dispatch_value}{x86_simd_note}"))
                elif cmake_old in content:
                    content = content.replace(cmake_old, cmake_new)
                    print(f"  [PATCH] install.sh: cmake SIMD flags injected [{arch_label}]"
                          + (f" dispatch={dispatch_value} +KLEIDICV" if is_arm64 else f" dispatch={dispatch_value}{x86_simd_note}"))
                else:
                    print("  [WARN]  install.sh: cmake marker not found; SIMD flags NOT injected")
                ...
                if wrapper_old in content:
                    ...
                    print("  [PATCH] install.sh: Injected --perf_threads into wrapper script")
                else:
                    print("  [WARN]  install.sh: wrapper marker not found; --perf_threads NOT injected")
    ...
    def _is_target_version_installed(self) -> bool:
        """Return True when the target OpenCV build artefacts are present."""
        ver = self._OPENCV_TARGET_VERSION
    ...
    def installed_perf_tests(self) -> set:
        """Return OpenCV perf test names that have matching opencv_perf_* binaries."""
        installed_base = Path.home() / ".phoronix-test-suite" / "installed-tests" / "pts" / self.benchmark
        tests = set()
        if not installed_base.exists():
            return tests

        for perf_bin in installed_base.rglob("opencv_perf_*"):
            if not perf_bin.is_file():
                continue
            name = perf_bin.name.removeprefix("opencv_perf_")
            if name:
                tests.add(name)
        return tests

    def patch_test_definition_to_installed_binaries(self) -> bool:
        """Drop test-definition entries whose opencv_perf_<test> binary was not built."""
        available_tests = self.installed_perf_tests()
        if not available_tests:
            print("  [WARN] No opencv_perf_* binaries found after install; keeping requested tests")
            return False

        keep_tests = self._KEEP_TESTS.intersection(available_tests)
        missing_tests = sorted(self._KEEP_TESTS - available_tests)
        if missing_tests:
            print(
                "  [WARN] Missing OpenCV perf binaries; removing from test-definition.xml: "
                + ", ".join(missing_tests)
            )

        if not keep_tests:
            print("  [ERROR] None of requested OpenCV tests have matching perf binaries")
            return False

        return self.patch_test_definition(keep_tests=keep_tests)
    ...
    def run(self):
        ...
        try:
            ...
            self.patch_test_definition_app_version()
            ...
            self.patch_test_definition_to_installed_binaries()
