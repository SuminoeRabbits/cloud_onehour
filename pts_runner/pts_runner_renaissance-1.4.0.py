...
class RenaissanceRunner:
    # Benchmarks excluded from the run due to known failures on certain CPU architectures.
    # Key: jar benchmark name (passed as $1 to the wrapper script)
    # Value: PTS display name (used in PTS failure log output)
    EXCLUDED_SUBTESTS = {
        'als': 'Apache Spark ALS',  # ValidationException on Zen4/AVX512: FP diff > tolerance
    }
    ...
    def patch_renaissance_wrapper(self):
        """Patch the renaissance wrapper script to skip excluded benchmarks gracefully.

        Benchmarks in EXCLUDED_SUBTESTS cause known failures (e.g. ValidationException
        due to floating-point precision differences on Zen4/AVX512).  The patch makes
        the wrapper exit 0 for those benchmarks so PTS does not record a failure.

        The wrapper is generated by install.sh as:
            java -jar renaissance-mit-0.16.0.jar -t 60 $@ --csv $LOG_FILE 2>&1
            echo $? > ~/test-exit-status

        After patching, excluded benchmarks hit a case branch that exits 0 before the
        java invocation, writing 0 to ~/test-exit-status so PTS sees a clean exit.
        """
        installed_dir = Path.home() / '.phoronix-test-suite' / 'installed-tests' / 'pts' / self.benchmark
        wrapper = installed_dir / 'renaissance'
        marker = installed_dir / '.renaissance_wrapper_patched'

        if not wrapper.exists():
            print(f"  [WARN] patch_renaissance_wrapper: wrapper not found at {wrapper}")
            return

        if marker.exists():
            print("  [INFO] patch_renaissance_wrapper: already patched (marker found)")
            return

        print("\n>>> Patching renaissance wrapper to skip excluded benchmarks...")
        print(f"  [INFO] Excluded benchmarks: {list(self.EXCLUDED_SUBTESTS.keys())}")

        original = wrapper.read_text()

        # Build one case entry per excluded benchmark
        case_entries = []
        for jar_name, display_name in self.EXCLUDED_SUBTESTS.items():
            case_entries.append(
                f'    {jar_name})\n'
                f'        echo "[SKIP] renaissance: \\"{jar_name}\\" excluded by pts_runner ({display_name})"\n'
        ...
        # Insert skip block immediately after the shebang line
        lines = original.split('\n')
        shebang_idx = next((i for i, l in enumerate(lines) if l.startswith('#!')), 0)
    ...
    def run_benchmark(self, num_threads):
        ...
        # If PTS reported failure, check whether ALL failing sub-tests are in EXCLUDED_SUBTESTS.
        # This is a fallback for the case where patch_renaissance_wrapper() was not applied
        # (e.g. first run before install, or marker deleted).
        if pts_test_failed:
            failed_subtests = self._parse_pts_failed_subtests(log_file)
            excluded_display = set(self.EXCLUDED_SUBTESTS.values())
            if failed_subtests and failed_subtests.issubset(excluded_display):
                print(f"\n[INFO] PTS failure(s) due to excluded sub-test(s): {failed_subtests}")
                print("[INFO] Overriding pts_test_failed → False (all failures are excluded benchmarks)")
