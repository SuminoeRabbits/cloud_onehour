...
- Software Dependencies:
  ...
  * Libtool
  * Linux AIO
- Estimated Install Time: 25 Seconds
- Environment Size: 30 MB
- Test Type: System
- Supported Platforms: Linux

Test Characteristics:
- Multi-threaded: Yes (CPU and memory tests scale with cores)
- Honors CFLAGS/CXXFLAGS: Yes
- Notable Instructions: MMX, SSE, SSE2 (can use AVX, AVX2, FMA, AVX512 with compiler flags)
...
class SysbenchRunner:
    ...
    def install_benchmark(self):
        """
        Install sysbench-1.1.0 with GCC-14 native compilation.

        Note: Unlike coremark, sysbench does NOT need reinstallation for each thread count
        because it supports runtime thread configuration via --threads argument.
    ...
    def run_benchmark(self, num_threads):
        ...
        batch_env = f'{quick_env}NUM_CPU_CORES={num_threads} BATCH_MODE=1 SKIP_ALL_PROMPTS=1 DISPLAY_COMPACT_RESULTS=1 TEST_RESULTS_NAME=sysbench-{num_threads}threads TEST_RESULTS_IDENTIFIER=sysbench-{num_threads}threads'
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            f.write("="*80 + "\n")
            f.write(f"Sysbench 1.1.0 Benchmark Summary\n")
    ...
    def run(self):
        """Main execution flow."""
        print(f"{'='*80}")
        print(f"Sysbench 1.1.0 Benchmark Runner")
        print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Sysbench 1.1.0 Benchmark Runner',
...
Examples:
  ...
  %(prog)s 16        # Run with 16 threads (capped at vCPU if exceeded)
    ...
    # Run benchmark
    runner = SysbenchRunner(args.threads, quick_mode=args.quick)
    success = runner.run()
