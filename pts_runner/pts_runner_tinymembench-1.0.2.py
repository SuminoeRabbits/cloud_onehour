#!/usr/bin/env python3
"""
PTS Runner for tinymembench-1.0.2
...
class TinymembenchRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        """
        Initialize Tinymembench runner.

        Note: Tinymembench is single-threaded, thread_arg is ignored.

        Args:
            threads_arg: Ignored (tinymembench is single-threaded)
            quick_mode: If True, run tests once (FORCE_TIMES_TO_RUN=1) for development
        """
        self.benchmark = "tinymembench-1.0.2"
        ...
        # Tinymembench is single-threaded - always run on single CPU
        self.thread_list = [1]
