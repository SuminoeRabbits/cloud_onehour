#!/usr/bin/env python3
"""
PTS Runner for mt-dgemm-1.3.1

System Dependencies (from phoronix-test-suite info):
- Software Dependencies:
  * C/C++ Compiler Toolchain
- Estimated Install Time: 351 Seconds
- Download Size: 41.56 MB
- Environment Size: 1 MB
- Test Type: Processor
- Supported Platforms: Linux, Solaris, MacOSX, BSD

Test Characteristics:
- Multi-threaded: Yes (OpenMP parallelization via runtime thread control)
- Honors CFLAGS/CXXFLAGS: Yes
- Notable Instructions: AVX, FMA
- Possible with extra compiler flags: AVX512
...
- Measures: Sustained floating-point rate in GFLOP/s (higher is better)

Build/runtime notes:
- OpenBLAS is built by the PTS profile during installation.
- No additional FPU-specific OS packages are required beyond setup_pts.sh
  build utilities and unzip support already present in this project.
"""
...
class MtDgemmRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        """
        Initialize mt-dgemm benchmark runner.
    ...
    def get_compiler_env(self):
        """Return compiler environment, preferring GCC 14 when available."""
    ...
    def install_benchmark(self):
        ...
        # Patch install.sh before install so re-installs remain idempotent.
        ...
        if not self.patch_install_script(required=True):
            print("  [ERROR] Failed to apply mt-dgemm install.sh patch after install")
            sys.exit(1)

        print(f"  [OK] Installation completed and verified: {installed_dir}")

    def patch_install_script(self, required=False):
        """Patch install.sh for idempotent OpenBLAS staging and safer parallel builds."""
        candidates = [
        ...
        for install_sh in candidates:
            ...
            try:
                ...
                replacements = [
                    ("mkdir $HOME/openblas_", "mkdir -p $HOME/openblas_"),
                    (
                        "make NUM_THREADS=1024 USE_THREAD=1 USE_OPENMP=1 DYNAMIC_ARCH=1 -j $NUM_CPU_CORES",
                        "make NUM_THREADS=1024 USE_THREAD=1 USE_OPENMP=1 DYNAMIC_ARCH=1 -j ${NUM_CPU_CORES:-1}",
                    ),
                ]
                ...
                if patched:
                    install_sh.write_text(content if content.endswith("\n") else content + "\n")
                    print(f"  [OK] Patched install.sh: {install_sh}")
                    patched_any = True
                elif original_content == content:
                    print(f"  [INFO] install.sh already patched or no patch needed: {install_sh}")
                    patched_any = True
            except Exception as e:
    ...
    def run_benchmark(self, num_threads):
        ...
        for cmd in remove_cmds:
            subprocess.run(['bash', '-c', f'echo "y" | {cmd}'], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def export_results(self):
        ...
        for num_threads in self.thread_list:
            ...
            # PTS removes dots from directory names, so "mt-dgemm-1.3.1" becomes "mt-dgemm-131"
            result_dir_name = result_name.replace('.', '')
            result_dir = pts_results_dir / result_dir_name
    ...
    def generate_summary(self):
        ...
        json_files = []
        if self.results_dir.exists():
            for f in self.results_dir.glob("*-thread.json"):
                match = re.match(r'^(\d+)-thread\.json$', f.name)
                if match:
                    json_files.append((int(match.group(1)), f))
        
        json_files.sort(key=lambda x: x[0])
        
        for num_threads, json_file in json_files:
            if json_file.exists():
...
def main():
    parser = argparse.ArgumentParser(
        description="ACES DGEMM Benchmark Runner",
    ...
    runner = MtDgemmRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
