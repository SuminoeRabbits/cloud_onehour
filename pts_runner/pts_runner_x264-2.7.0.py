...
- Software Dependencies:
    ...
    * yasm
    * nasm
- Estimated Install Time: (PTS managed)
- Environment Size: 2600 MB
- Test Type: Processor
- Supported Platforms: Linux, Windows, BSD, MacOSX

Test Characteristics:
- Multi-threaded: Yes (video encoding is highly parallel)
- THFix_in_compile: false - Thread count NOT fixed at compile time
- THChange_at_runtime: false - x264 auto-manages threads in this PTS profile
...
class PreSeedDownloader:
    ...
    def download_from_xml(self, benchmark_name, threshold_mb=96):
        ...
        Args:
            benchmark_name: Full benchmark name (e.g., "pts/x264-2.7.0")
...
class X264Runner:
    ...
    def dump_error_diagnostics(self, num_threads, log_file):
        ...
        2. x264 specific logs and error output
        ...
        with open(diag_file, 'w') as f:
            ...
            if installed_dir.exists():
                ...
                # x264 specific: check for build/compile errors
            ...
            # 3. Try to run x264 manually to capture error
            f.write("\n\n" + "-"*80 + "\n")
            f.write("[3] x264 Binary Test\n")
            f.write("-"*80 + "\n")

            x264_bin = installed_dir / "x264"
            if x264_bin.exists():
                f.write("\n[x264 --version]\n")
                result = subprocess.run(
                    [str(x264_bin), '--version'],
                    capture_output=True, text=True
                )
                ...
                f.write(f"x264 binary not found at {x264_bin}\n")
            ...
            if x264_bin.exists():
                f.write(f"\n[ldd {x264_bin}]\n")
                result = subprocess.run(['ldd', str(x264_bin)], capture_output=True, text=True)
                f.write(result.stdout)
    ...
    def install_benchmark(self):
        ...
        Note: x264 auto-detects CPU cores. Since THFix_in_compile=false and
        ...
        if not self.ensure_7z_available():
            print("  [ERROR] 7z is required to extract input files for x264")
            sys.exit(1)
        ...
        # [Pattern 5] Pre-download large files from downloads.xml
    ...
    def run_benchmark(self, num_threads):
        ...
        Note: x264 auto-detects CPU cores. We use taskset to limit CPU visibility
        since THChange_at_runtime=false (x264 doesn't accept runtime thread arguments).
        """
        print(f"\n{'='*80}")
        ...
        # Build PTS command
        # Note: x264 auto-detects, so we don't pass NUM_CPU_CORES
        ...
        # Always use taskset for x264 to control CPU visibility
        # x264 will auto-detect the CPUs available in the taskset mask
        cpu_list = self.get_cpu_affinity_list(num_threads)
        pts_base_cmd = f'taskset -c {cpu_list} phoronix-test-suite batch-run {self.benchmark_full}'
        cpu_info = f"CPU affinity (taskset): {cpu_list} - x264 will auto-detect {num_threads} CPU(s)"

        # CRITICAL: Environment variables MUST come BEFORE perf stat
        # Note: NO NUM_CPU_CORES for x264 (auto-detect only)
        if self.perf_events:
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            f.write("="*80 + "\n")
            f.write("x264 2.7.0 Benchmark Summary\n")
    ...
    def run(self):
        """Main execution flow."""
        print(f"{'='*80}")
        print("x264 2.7.0 Benchmark Runner")
        print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description="x264 2.7.0 Benchmark Runner",
    ...
    runner = X264Runner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
