...
class JavaJmhRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        """
        Initialize JMH benchmark runner.
    ...
    def install_benchmark(self):
        """
        Install java-jmh-1.0.1 with native compilation.

        Note: Unlike coremark, JMH does NOT need reinstallation for each thread count
        because it supports runtime thread configuration via -t max argument.
        ...
        # Apply specific fixes and re-run install if needed
        ...
        # Verify installation by checking if test is actually installed AND jar exists
        ...
        # Check for the jar file which confirms successful build
        pts_home = Path.home() / '.phoronix-test-suite'
        jar_path = pts_home / 'installed-tests' / 'pts' / self.benchmark / 'test' / 'target' / 'benchmarks.jar'

    def fix_benchmark_specific_issues(self):
        """
        Java 25環境でのビルド不具合(META-INF/BenchmarkListの欠落)を修正。
        install.sh内のpom.xml生成ロジックに -proc:full を追加することで、
        アノテーションプロセッサーを強制的に動作させるパッチを適用する。

        Returns:
            bool: True if a patch was applied, False otherwise.
        """
        ...
        try:
            ...
            if '-proc:full' in content:
                return False

            print(f"  [FIX] Applying patch to install.sh for Java 25 annotation processing...")
            
            # The replacement target in install.sh (generated by pts archetype)
            # We want to insert the compiler args after the <target> tag in the sed command or before mvn clean install
            # The original install.sh usually has: mvn clean install
            
            # We will use a robust replacement that handles the clean install line
            if 'mvn clean install' in content:
                # Use single quotes for the sed command string to avoid python f-string/escaping issues
                patch_code = """
# Patch pom.xml for Java 25 annotation processing
# Using single quotes to prevent variable expansion
sed -i '/<target>${javac.target}<\\/target>/a \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ <compilerArgs>\\n\\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ <arg>-proc:full</arg>\\n\\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ \\ </compilerArgs>' pom.xml

mvn clean install
"""
                new_content = content.replace('mvn clean install', patch_code.strip())
        ...
        if verify_result.returncode != 0 or 'not found' in verify_result.stdout.lower():
            ...
            print(f"  [ERROR] Build verification failed - jar file not found: {jar_path}")
            print(f"  [INFO] This usually means 'mvn' (Maven) is missing or the build failed.")
            print(f"  [INFO] Please run the setup script to install dependencies:")
            print(f"         ./cloud_onehour/scripts/setup_jdk25.sh")
            # Check for maven
            if not shutil.which('mvn'):
                print(f"  [CRITICAL] 'mvn' command not found in PATH!")
            sys.exit(1)
    ...
    def run_benchmark(self, num_threads):
        ...
        JMH uses -t max to utilize all available threads at runtime.
        Thread scaling is controlled via NUM_CPU_CORES environment variable.
        ...
        batch_env = f'{quick_env}BATCH_MODE=1 SKIP_ALL_PROMPTS=1 DISPLAY_COMPACT_RESULTS=1 TEST_RESULTS_NAME=java-jmh-{num_threads}threads TEST_RESULTS_IDENTIFIER=java-jmh-{num_threads}threads'
    ...
    def export_results(self):
        ...
        for num_threads in self.thread_list:
            result_name = f"java-jmh-{num_threads}threads"
...
def main():
    parser = argparse.ArgumentParser(
        description='JMH Benchmark Runner',
...
Examples:
  ...
  %(prog)s 16        # Run with 16 threads (capped at vCPU if exceeded)
    ...
    # Run benchmark
    runner = JavaJmhRunner(args.threads, quick_mode=args.quick)
    success = runner.run()
