...
- Software Dependencies:
  ...
  * cmake
  * hdf5
- Estimated Install Time: ~10-20 Minutes (toolchain build + CP2K compile)
- Environment Size: ~244 MB
- Test Type: Processor
- Supported Platforms: Linux (x86_64)

Test Characteristics:
- Multi-threaded: Yes (MPI ranks via mpirun, OMP_NUM_THREADS=1)
...
class CP2KRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        """
        Initialize CP2K benchmark runner.
        ...
        if self.arch not in ("x86_64", "amd64"):
            print(f"  [WARN] Upstream cp2k-1.5.0 profile is x86_64-only. Current arch: {self.arch}")
    ...
    def install_benchmark(self):
        """Install CP2K with source prefetch and install.sh patching."""
        print("\n>>> Checking for downloadable source archives...")
        self.fetch_test_profile()
        downloader = PreSeedDownloader()
        downloader.download_from_xml(self.benchmark_full, threshold_mb=64)
    ...
    def run_benchmark(self, num_threads):
        """Run CP2K with taskset-based affinity and NUM_CPU_CORES-driven MPI scaling."""
        self.results_dir.mkdir(parents=True, exist_ok=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="PTS Runner for cp2k-1.5.0 (CP2K Molecular Dynamics)",
    ...
    runner = CP2KRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
