...
- System Dependencies:
  * libuv headers
  * hwloc headers
- Estimated Install Time: ~25 MB environment / build time depends on CPU
...
- Multi-threaded: Yes
- Honors CFLAGS/CXXFLAGS: Yes (CMake-based build)
- Upstream Source: XMRig 6.25.0
- Result Scale: H/s (higher is better)
- THFix_in_compile: false - Thread count NOT fixed at compile time
- THChange_at_runtime: true - Runtime thread configuration via --threads=$NUM_CPU_CORES
"""
...
class XmrigRunner:
    # XMRig upstream profile has 6 variants x 2 hash counts = 12 tests.
    # Keep this list small enough for cloud runs while preserving distinct
    # algorithm families. Edit only these two sets to add/remove sub-tests.
    KEEP_VARIANTS = {
        "Monero",
        "KawPow",
        "CryptoNight-Heavy",
    ...
    def check_build_dependencies(self):
        """Check XMRig native build dependencies before invoking PTS."""
        missing = []
        ...
        checks = [
            ("libuv", "uv.h", "libuv1-dev"),
            ("hwloc", "hwloc.h", "libhwloc-dev"),
            ("openssl", "openssl/ssl.h", "libssl-dev"),
        ]
        ...
        for pkg_name, header, apt_package in checks:
            header_found = any((root / header).exists() for root in include_roots)
    ...
    def patch_test_definition(self):
        ...
        pts_home = Path.home() / ".phoronix-test-suite"
        candidate_paths = [
            pts_home / "test-profiles" / "pts" / self.benchmark / "test-definition.xml",
        ...
        for test_def_path in candidate_paths:
            ...
            try:
                tree.write(test_def_path, encoding="utf-8", xml_declaration=True)
                patched_any = True
                print(
                    "  [PATCH] XMRig test-definition filtered "
                    f"({test_def_path}): variants={sorted(self.KEEP_VARIANTS)}, "
                    f"hash_counts={sorted(self.KEEP_HASH_COUNTS)}"
                )
            except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="Run PTS XMRig benchmark")
    ...
    runner = XmrigRunner(threads_arg=threads, quick_mode=args.quick)
    ok = runner.run()
