...
- Environment Size: ~1950 MB (includes large YUV video input files)
- Test Type: Processor
- Supported Platforms: Linux, MacOSX, BSD

Test Characteristics:
- Upstream PTS profile: pts/aom-av1-3.12.0
- AppVersion: AOM AV1 3.12
- Multi-threaded: Yes (aomenc --threads=$NUM_CPU_CORES)
- Honors CFLAGS/CXXFLAGS: Yes, through CMake/toolchain environment
- Notable Instructions: SIMD paths selected by libaom/build environment
...
- Thread cap: AOM profile caps NUM_CPU_CORES at 64 before invoking aomenc

Test Options:
- Encoder modes: Speed 11/10/9/8/6 Realtime, Speed 6/4/0 Two-Pass
- Inputs: Bosphorus 1080p, Bosphorus 4K

Metric: Frames Per Second (FPS), higher is better
Input: Bosphorus 1080p/4K YUV4MPEG video samples
Runs: 3 iterations per configuration
"""
...
class AomAv1Runner:
    def __init__(self, threads_arg=None, quick_mode=False):
        ...
        self.thread_cap = min(self.vcpu_count, 64)
        self.machine_name = os.environ.get('MACHINE_NAME', os.uname().nodename)
        self.os_name = self.get_os_name()

        # Determine thread execution mode
        # AOM AV1's PTS wrapper reads NUM_CPU_CORES and caps it at 64.
        # Start with the standard 4-point scaling pattern, then apply the AOM cap.
        if threads_arg is None:
            ...
            self.thread_list = sorted(list(set([min(t, self.thread_cap) for t in self.thread_list])))
        else:
            # Fixed mode: single thread count
            n = min(threads_arg, self.vcpu_count)
            n = min(n, self.thread_cap)
            self.thread_list = [n]
    ...
    def ensure_build_tools_available(self):
        ...
        aom-av1-3.12.0 builds libaom with CMake.
        """
        required_commands = ["7z", "cmake", "make", "perl", "yasm"]
    ...
    def clean_pts_cache(self):
        ...
        # NOTE: Do NOT clean test profiles - they may contain manual fixes
    ...
    def install_benchmark(self):
        """
        Install aom-av1-3.12.0.

        aom-av1 does NOT need reinstallation for each thread count.
        Thread count is controlled at runtime via NUM_CPU_CORES and taskset.

        Build system: CMake + make.
        THFix_in_compile=false, THChange_at_runtime=true.
        """
        ...
        # AOM AV1 uses CMake + make; NUM_CPU_CORES is consumed by install.sh.
        nproc = os.cpu_count() or 1
        install_cmd = (
            f'NUM_CPU_CORES={nproc} MAKEFLAGS="-j{nproc}" '
    ...
    def run_benchmark(self, num_threads):
        ...
        Thread control for aom-av1 is done via NUM_CPU_CORES and taskset.
        The PTS wrapper calls aomenc --threads=$NUM_CPU_CORES and caps it at 64.

        Args:
            num_threads: Number of threads (= number of cores exposed via taskset)
        """
        print(f"\n{'='*80}")
        ...
        # Environment variables to suppress all prompts and set runtime thread count.
        ...
        batch_env = (
            f'{quick_env}'
            f'NUM_CPU_CORES={num_threads} BATCH_MODE=1 SKIP_ALL_PROMPTS=1 DISPLAY_COMPACT_RESULTS=1 '
        ...
        if num_threads >= self.vcpu_count:
            ...
            # Partial vCPU mode - use taskset to limit available cores
            # aomenc should only see/use the cores exposed by taskset
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            f.write("="*80 + "\n")
            f.write("AOM AV1 3.12.0 Benchmark Summary\n")
            ...
            f.write("="*80 + "\n")
            f.write("Summary Table (FPS, higher is better)\n")
            f.write("="*80 + "\n")
    ...
    def run(self):
        ...
        # Install benchmark ONCE (runtime thread configuration via taskset, no reinstall needed)
...
def main():
    parser = argparse.ArgumentParser(
        description="AOM AV1 3.12.0 Benchmark Runner",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s                  # Scaling mode: [nproc/4, nproc/2, nproc*3/4, nproc]
  %(prog)s 16               # Fixed mode: 16 threads via taskset
  %(prog)s --threads 8      # Fixed mode: 8 threads via taskset
  %(prog)s --quick          # Quick mode: FORCE_TIMES_TO_RUN=1 (development)
  %(prog)s 16 --quick       # Fixed + quick mode
"""
    )
    ...
    runner = AomAv1Runner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
