#!/usr/bin/env python3
"""
PTS Runner for glibc-bench-1.9.0

System Dependencies (from phoronix-test-suite info):
- Software Dependencies:
  * C/C++ Compiler Toolchain
    * Bison
    * gawk (system dependency)
- Estimated Install Time: 466 Seconds
- Environment Size: 100 MB
- Test Type: OS
- Supported Platforms: Linux

Test Characteristics:
- Multi-threaded: No (selects benchtests case, no runtime thread option exposed by profile)
- Honors CFLAGS/CXXFLAGS: Yes
- Notable Instructions: N/A
- THFix_in_compile: false - Thread count NOT fixed at compile time
- THChange_at_runtime: false - No runtime thread configuration option exposed by this profile
"""
...
class GlibcBenchRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        ...
        """
        self.benchmark = "glibc-bench-1.9.0"
        ...
        self.runner_lock_file = Path("/tmp") / f"cloud_onehour_{self.benchmark}.lock"
    ...
    def acquire_runner_lock(self):
        """Prevent concurrent execution of the same benchmark runner."""
        if self.runner_lock_file.exists():
            try:
                old_pid = int(self.runner_lock_file.read_text().strip() or "0")
            except Exception:
                old_pid = 0

            if old_pid > 0 and Path(f"/proc/{old_pid}").exists():
                print(f"[ERROR] Another {self.benchmark} runner is already running (PID: {old_pid})")
                print("[INFO] Wait for the previous run to finish, or stop it before retrying")
                sys.exit(1)

            print(f"[WARN] Stale lock detected, removing: {self.runner_lock_file}")
            self.runner_lock_file.unlink(missing_ok=True)

        self.runner_lock_file.write_text(str(os.getpid()))

    def release_runner_lock(self):
        """Release benchmark runner lock file."""
        self.runner_lock_file.unlink(missing_ok=True)

    def clear_stale_pts_run_lock(self):
        """Remove stale PTS run_lock left by aborted runs."""
        run_lock = Path.home() / '.phoronix-test-suite' / 'installed-tests' / 'pts' / self.benchmark / 'run_lock'
        if run_lock.exists():
            print(f"  [WARN] Removing stale PTS run_lock: {run_lock}")
        ...
        if installed_dir.exists():
            ...
            for attempt in range(1, 4):
                try:
                    ...
                    print(f"  [WARN] rmtree attempt {attempt}/3 failed: {e}")
                    time.sleep(0.3 * attempt)

            if not removed and installed_dir.exists():
                print("  [INFO] Falling back to rm -rf for stale files")
                result = subprocess.run(
                    ['rm', '-rf', str(installed_dir)],
                    capture_output=True,
                    text=True
                )
                if result.returncode == 0 and not installed_dir.exists():
                    removed = True
                else:
                    print(f"  [ERROR] Failed to remove installed test directory: {installed_dir}")
                    if result.stderr:
                        print(f"  [ERROR] rm stderr: {result.stderr.strip()}")
                    sys.exit(1)
    ...
    def install_benchmark(self):
        ...
        install_cmd = f'env -u LD_LIBRARY_PATH NUM_CPU_CORES={nproc} phoronix-test-suite batch-install {self.benchmark_full}'
        ...
        if verify_result.returncode != 0:
            print("  [WARN] test-installed check failed, validating with info/test-installed policy...")

        install_status = get_install_status(self.benchmark_full, self.benchmark)
        if not install_status["already_installed"]:
            print("  [ERROR] Installation verification failed: PTS still reports benchmark as not installed")
            print(
                f"  [ERROR] Install status -> info:{install_status['info_installed']}, "
                f"test-installed:{install_status['test_installed_ok']}, dir:{install_status['installed_dir_exists']}"
            )
            print(f"  [INFO] Try manual check: phoronix-test-suite info {self.benchmark_full}")
            sys.exit(1)
    ...
    def export_results(self):
        ...
        for num_threads in self.thread_list:
            ...
            # PTS removes dots from directory names, e.g. "glibc-bench-1.9.0" becomes "glibc-bench-190"
            result_dir_name = result_name.replace('.', '')
            result_dir = pts_results_dir / result_dir_name
...
def main():
    parser = argparse.ArgumentParser(
        description="Glibc Benchmarks 1.9.0 Runner",
    ...
    runner = GlibcBenchRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
