...
class BuildLinuxKernelRunner:
    ...
    def run_benchmark(self, num_threads):
        ...
        batch_env = f'BATCH_MODE=1 SKIP_ALL_PROMPTS=1 DISPLAY_COMPACT_RESULTS=1 TEST_RESULTS_NAME=build-linux-kernel-{num_threads}threads TEST_RESULTS_IDENTIFIER=build-linux-kernel-{num_threads}threads'
...
Examples:
  ...
  %(prog)s 16        # Run with 16 threads (capped at vCPU if exceeded)
    ...
    # Run benchmark
    runner = BuildLinuxKernelRunner(args.threads)
    success = runner.run()
