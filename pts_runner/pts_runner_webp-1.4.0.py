...
- Environment Size: ~25 MB
- Test Type: Processor
- Supported Platforms: Linux, MacOSX, Windows

Test Characteristics:
- Multi-threaded: Yes (-mt flag enables all-core worker threads via libwebp internal pool)
- Honors CFLAGS/CXXFLAGS: Yes
- Notable Instructions: SSE2, NEON (via compiler intrinsics, not nasm/yasm)
- THFix_in_compile: false - Thread count NOT fixed at compile time
- THChange_at_runtime: false - cwebp -mt uses all available cores; thread count
                                controlled via taskset (CPU affinity)

Test Options (5 configurations):
1. Default settings
2. Quality 100
3. Quality 100 with highest compression (-m 6)
4. Quality 100 with lossless encoding
5. Quality 100 lossless with highest compression (-m 6)

Metric: Megapixels per second (MP/s), higher is better
Input: 6000x4000 pixel JPEG photograph (sample-photo-6000x4000-1.zip)
...
class WebpRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        ...
        # webp's cwebp -mt uses all available cores; thread scaling is done via taskset.
    ...
    def ensure_build_tools_available(self):
        ...
        webp-1.4.0 uses autotools (configure + make); no cmake/ninja/nasm/yasm needed.
        """
    ...
    def install_benchmark(self):
        ...
        webp does NOT need reinstallation for each thread count.
        Thread count is controlled at runtime via taskset (CPU affinity).

        Build system: autotools (configure + make) — no cmake/ninja required.
        ...
        # webp uses autotools (configure + make); no cmake needed.
    ...
    def run_benchmark(self, num_threads):
        ...
        Thread control for webp is done via taskset (CPU affinity).
        cwebp -mt uses all available CPU cores; no NUM_CPU_CORES env var.
        ...
        # Note: NUM_CPU_CORES is NOT set for webp; thread count is controlled via taskset
        ...
        if num_threads >= self.vcpu_count:
            ...
            # cwebp -mt will use only the cores exposed by taskset
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            f.write("="*80 + "\n")
            f.write("WebP 1.4.0 Benchmark Summary\n")
    ...
    def run(self):
        """Main execution flow."""
        print(f"{'='*80}")
        print("WebP 1.4.0 Benchmark Runner")
        print(f"{'='*80}")
        ...
        print("[INFO] Thread mode: taskset-based (cwebp -mt uses all available cores)")
...
def main():
    parser = argparse.ArgumentParser(
        description="WebP 1.4.0 Benchmark Runner",
...
"""
    ...
    runner = WebpRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
