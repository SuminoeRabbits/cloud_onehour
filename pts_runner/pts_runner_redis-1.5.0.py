#!/usr/bin/env python3
"""
PTS Runner for redis-1.5.0

System Dependencies (from phoronix-test-suite info):
- Software Dependencies:
  * C/C++ Compiler Toolchain
- Estimated Install Time: 108 Seconds
- Environment Size: 64 MB
- Test Type: System
...
class RedisRunner:
    ...
    def ensure_runtime_wrapper(self):
        """
        Fix the installed PTS wrapper so it points at the actual installed test path.
        redis-1.5.0's install script hardcodes ~/redis-8.6.0, but PTS installs under
        ~/.phoronix-test-suite/installed-tests/pts/<benchmark>/redis-8.6.0.
        """
        installed_dir = Path.home() / '.phoronix-test-suite' / 'installed-tests' / 'pts' / self.benchmark
        wrapper_path = installed_dir / 'redis'
        source_dir = installed_dir / 'redis-8.6.0'

        if not wrapper_path.exists():
            print(f"  [WARN] Runtime wrapper not found: {wrapper_path}")
            return False

        if not source_dir.exists():
            print(f"  [WARN] Redis source directory not found: {source_dir}")
            return False

        try:
            content = wrapper_path.read_text()
            expected_line = "cd ~/redis-8.6.0"
            fixed_line = f'cd "{source_dir}"'

            if expected_line in content:
                wrapper_path.write_text(content.replace(expected_line, fixed_line, 1))
                print(f"  [OK] Patched Redis runtime wrapper: {wrapper_path}")
            else:
                print("  [INFO] Redis runtime wrapper already patched")
            return True
        except Exception as e:
            print(f"  [WARN] Failed to patch Redis runtime wrapper: {e}")
            return False

    def get_runtime_paths(self):
        """Return installed runtime paths used by redis-1.5.0."""
        installed_dir = Path.home() / '.phoronix-test-suite' / 'installed-tests' / 'pts' / self.benchmark
        source_dir = installed_dir / 'redis-8.6.0'
        return {
            ...
            "server_path": source_dir / 'src' / 'redis-server',
            "benchmark_path": source_dir / 'src' / 'redis-benchmark',
        }

    def is_tcp_port_open(self, host, port, timeout=0.5):
    ...
    def debug_runtime_state(self, thread_dir):
        """
        Emit debug information before running PTS so runtime issues are visible early.
        Returns True if a manual redis-server preflight succeeds, False otherwise.
        """
        print(f"\n{'='*80}")
        print(">>> Redis Runtime Debug")
        print(f"{'='*80}")

        runtime = self.get_runtime_paths()
        wrapper_path = runtime["wrapper_path"]
        source_dir = runtime["source_dir"]
        server_path = runtime["server_path"]
        benchmark_path = runtime["benchmark_path"]

        print(f"  [DEBUG] Installed dir: {runtime['installed_dir']}")
        print(f"  [DEBUG] Wrapper path: {wrapper_path} (exists={wrapper_path.exists()})")
        print(f"  [DEBUG] Source dir: {source_dir} (exists={source_dir.exists()})")
        print(f"  [DEBUG] redis-server: {server_path} (exists={server_path.exists()})")
        print(f"  [DEBUG] redis-benchmark: {benchmark_path} (exists={benchmark_path.exists()})")
        print(f"  [DEBUG] Port 6379 open before run: {self.is_tcp_port_open('127.0.0.1', 6379)}")

        if wrapper_path.exists():
            try:
                wrapper_content = wrapper_path.read_text()
                print("  [DEBUG] Wrapper contents:")
                for line in wrapper_content.splitlines():
                    print(f"    {line}")
            except Exception as e:
                print(f"  [WARN] Failed to read wrapper: {e}")

        if not server_path.exists():
            print("  [WARN] redis-server binary missing; skipping preflight")
            return False

        preflight_port = 6380
        preflight_conf = thread_dir / "redis-preflight.conf"
        preflight_stdout = thread_dir / "redis-preflight.stdout.log"
        preflight_stderr = thread_dir / "redis-preflight.stderr.log"
        preflight_server_log = thread_dir / "redis-preflight.server.log"
        preflight_conf.write_text(
            "\n".join([
                "bind 127.0.0.1",
                f"port {preflight_port}",
                "protected-mode no",
                "save \"\"",
                "appendonly no",
                "daemonize no",
                "tcp-keepalive 0",
                f'logfile "{preflight_server_log}"',
                "",
            ])
        )

        print(f"  [DEBUG] Running redis-server preflight on port {preflight_port}")
        with open(preflight_stdout, 'w') as stdout_f, open(preflight_stderr, 'w') as stderr_f:
            process = subprocess.Popen(
                [str(server_path), str(preflight_conf)],
                cwd=source_dir,
                stdout=stdout_f,
                stderr=stderr_f,
                text=True
            ...
            for _ in range(10):
                if process.poll() is not None:
                    break
                if self.is_tcp_port_open('127.0.0.1', preflight_port):
                    port_open = True
                    break
                time.sleep(0.5)

            if port_open:
                print(f"  [OK] redis-server preflight accepted connections on 127.0.0.1:{preflight_port}")
            else:
                print(f"  [WARN] redis-server preflight did not open port {preflight_port}")
                print(f"  [DEBUG] preflight return code: {process.poll()}")
                print(f"  [DEBUG] preflight stdout: {preflight_stdout}")
                print(f"  [DEBUG] preflight stderr: {preflight_stderr}")
                print(f"  [DEBUG] preflight server log: {preflight_server_log}")
    ...
    def debug_wrapper_execution(self, thread_dir, num_threads):
        """
        Run the installed Redis wrapper directly and check whether it actually
        opens 127.0.0.1:6379 before PTS invokes it.
        """
        print(f"\n{'='*80}")
        print(">>> Redis Wrapper Debug")
        print(f"{'='*80}")

        runtime = self.get_runtime_paths()
        wrapper_path = runtime["wrapper_path"]
        source_dir = runtime["source_dir"]
        wrapper_stdout = thread_dir / "redis-wrapper-debug.stdout.log"
        wrapper_stderr = thread_dir / "redis-wrapper-debug.stderr.log"
        wrapper_bench = thread_dir / "redis-wrapper-debug.benchmark.log"
        wrapper_conf_copy = thread_dir / "redis-wrapper-debug.redis.conf"
        wrapper_server_log_copy = thread_dir / "redis-wrapper-debug.server.log"

        if not wrapper_path.exists():
            print(f"  [WARN] Wrapper missing, cannot debug execution: {wrapper_path}")
            return False

        env = os.environ.copy()
        env["NUM_CPU_CORES"] = str(num_threads)
        env["LOG_FILE"] = str(wrapper_bench)

        print(f"  [DEBUG] Executing wrapper directly: {wrapper_path}")
        print(f"  [DEBUG] Wrapper benchmark log: {wrapper_bench}")

        with open(wrapper_stdout, 'w') as stdout_f, open(wrapper_stderr, 'w') as stderr_f:
            process = subprocess.Popen(
                [str(wrapper_path), "-t", "get", "-c", "50", "-n", "1000", "-P", "1", "-q", "--csv"],
            ...
            if port_open:
                print("  [OK] Wrapper execution opened 127.0.0.1:6379")
            else:
                print("  [WARN] Wrapper execution did not open 127.0.0.1:6379")

            time.sleep(1.0)
            running_after_probe = process.poll() is None
            print(f"  [DEBUG] Wrapper still running after probe: {running_after_probe}")
            print(f"  [DEBUG] Wrapper return code (if exited): {process.poll()}")
        ...
        runtime_conf = source_dir / "redis.conf"
        runtime_server_log = source_dir / "redis.log"
        if runtime_conf.exists():
            shutil.copy2(runtime_conf, wrapper_conf_copy)
            print(f"  [DEBUG] Saved generated redis.conf: {wrapper_conf_copy}")
        else:
            print(f"  [WARN] Wrapper did not generate redis.conf in {source_dir}")

        if runtime_server_log.exists():
            shutil.copy2(runtime_server_log, wrapper_server_log_copy)
            print(f"  [DEBUG] Saved redis server log: {wrapper_server_log_copy}")
        else:
            print(f"  [INFO] No redis.log found in {source_dir}")

        print(f"  [DEBUG] Wrapper stdout: {wrapper_stdout}")
        print(f"  [DEBUG] Wrapper stderr: {wrapper_stderr}")
        return port_open
    ...
    def run_benchmark(self, num_threads):
        ...
        self.ensure_runtime_wrapper()
        preflight_ok = self.debug_runtime_state(thread_dir)
        if not preflight_ok:
            print("  [WARN] Redis preflight failed before PTS execution; expect benchmark failure")
        wrapper_ok = self.debug_wrapper_execution(thread_dir, num_threads)
        if not wrapper_ok:
            print("  [WARN] Wrapper debug failed before PTS execution; issue is likely inside the PTS wrapper path")
    ...
    def generate_summary(self):
        ...
        with open(summary_log, 'w') as f:
            ...
            for result in all_results:
                unit_str = result['unit'] if result['unit'] is not None else "N/A"
                desc_str = result['description'] if result['description'] is not None else "N/A"
                ...
                f.write(f"  Average: {val_str} {unit_str}\\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Redis 8.6 Benchmark Runner (Single-thread mode)",
    ...
    runner = RedisRunner(threads_arg=threads, quick_mode=args.quick)
    success = runner.run()
