#!/usr/bin/env python3
"""
PTS Runner for valkey-1.1.0

System Dependencies (from phoronix-test-suite info):
- Software Dependencies:
  * C/C++ Compiler Toolchain
- Estimated Install Time: ~240 Seconds
- Environment Size: ~55 MB
- Test Type: System
- Supported Platforms: Linux

Test Characteristics:
- Multi-threaded: Yes (server-side IO threads + benchmark client threads)
- Honors CFLAGS/CXXFLAGS: Yes
- Notable Instructions: N/A
- THFix_in_compile: false - Thread count NOT fixed at compile time
- THChange_at_runtime: true - Runtime thread configuration via NUM_CPU_CORES env var
  * valkey-server is started with --io-threads NUM_CPU_CORES (server-side MT)
  * valkey-benchmark is run with --threads NUM_CPU_CORES (client-side MT)

Multi-Thread Notes for valkey-1.1.0:
  Unlike valkey-1.0.0 which only exercised client-side threading, valkey-1.1.0
  enables full multi-threaded I/O on the server side as well. The PTS launcher
  reads NUM_CPU_CORES to configure both server and benchmark thread counts.
  The underlying Valkey source directory is detected dynamically at runtime to
  support future Valkey server version upgrades within the same PTS profile.
"""
...
class PreSeedDownloader:
    ...
    def download_from_xml(self, benchmark_name, threshold_mb=96):
        ...
        Args:
            benchmark_name: Full benchmark name (e.g., "pts/valkey-1.1.0")
...
class ValkeyRunner:
    def __init__(self, threads_arg=None, quick_mode=False):
        """
        Initialize Valkey 1.1.0 benchmark runner.
        ...
        # Thread list setup
        # valkey-1.1.0: THChange_at_runtime=true, TH_scaling=env:NUM_CPU_CORES
        # Scale across [nproc/4, nproc/2, nproc*3/4, nproc] to capture MT throughput curve
    ...
    def _find_valkey_src_dir(self, installed_dir):
        """
        Dynamically locate the Valkey source directory within the installed test directory.

        The underlying Valkey server version bundled with a PTS test profile can vary
        (e.g., valkey-8.0.0 for pts/valkey-1.0.0, valkey-8.1.0 for pts/valkey-1.1.0).
        This method searches for the first valkey-X.Y.Z directory rather than
        hardcoding the version, making the runner robust across Valkey server upgrades.

        Args:
            installed_dir: Path to the installed test directory
                           (~/.phoronix-test-suite/installed-tests/pts/valkey-1.1.0)

        Returns:
            Path: The valkey source directory, or None if not found
        """
        candidates = sorted(installed_dir.glob("valkey-*/"))
        if candidates:
            src_dir = candidates[0]
            print(f"  [INFO] Detected Valkey source directory: {src_dir.name}")
            return src_dir

        # Fallback: search for valkey-server binary anywhere under installed_dir
        server_bins = list(installed_dir.glob("*/src/valkey-server"))
        if server_bins:
            return server_bins[0].parent.parent

        return None

    def install_benchmark(self):
        """Install benchmark with error detection and verification."""
        # [Pattern 5] Pre-download large files from downloads.xml (Size > 96MB)
        ...
        # Dynamically detect the Valkey source directory.
        # valkey-1.1.0 may bundle a different server version than valkey-8.0.0.
        valkey_src_dir = self._find_valkey_src_dir(installed_dir)
        if valkey_src_dir is None:
            print("  [ERROR] Could not locate Valkey source directory under installed test")
            print(f"  [ERROR] Searched in: {installed_dir}")
            print("  [ERROR] Expected pattern: valkey-X.Y.Z/")
            sys.exit(1)

        valkey_server = valkey_src_dir / "src" / "valkey-server"
        if not valkey_server.exists():
            print(f"  [ERROR] valkey-server binary not found: {valkey_server}")
        ...
        # The bundled PTS valkey launcher may use a hardcoded ~/valkey-X.Y.Z path.
        # Satisfy this dependency by creating a symlink from ~/valkey-X.Y.Z to the
        # actual installed source tree so the launcher can find the binary.
        launcher_expected_src = Path.home() / valkey_src_dir.name
        try:
            ...
            print(f"  [WARN] Valkey launcher may fail if it expects ~/{valkey_src_dir.name}")
    ...
    def _get_valkey_rdb_path(self):
        """
        Determine the dump.rdb path used by the running valkey-server instance.

        For valkey-1.1.0, the underlying server version is detected dynamically.
        The PTS launcher typically starts valkey-server from ~/valkey-X.Y.Z/,
        so dump.rdb will appear there.

        Returns:
            Path or None: Path to dump.rdb if the source dir can be determined.
        """
        pts_home = Path.home() / '.phoronix-test-suite'
        installed_dir = pts_home / 'installed-tests' / 'pts' / self.benchmark
        if installed_dir.exists():
            valkey_src_dir = self._find_valkey_src_dir(installed_dir)
            if valkey_src_dir:
                # Symlink target under ~/ is where valkey-server actually runs
                return Path.home() / valkey_src_dir.name / "dump.rdb"
        return None
    ...
    def _run_direct_launcher_matrix(self, num_threads, launcher_path, log_file, stdout_log, freq_start_file, freq_end_file):
        ...
        print(f"  [MT]   NUM_CPU_CORES={num_threads} -> server io-threads + benchmark threads")
        ...
        # Resolve RDB path once (dynamic, based on installed Valkey version)
        valkey_rdb = self._get_valkey_rdb_path()

        direct_results = []
        with open(log_file, 'w') as log_f, open(stdout_log, 'a') as stdout_f:
            for index, case in enumerate(cases, start=1):
                # Clean up any existing RDB file to prevent long loading times
                # that exceed the hardcoded sleep in the PTS launcher script.
                # valkey-server with io-threads enabled writes to dump.rdb in its
                # working directory (~/valkey-X.Y.Z/), so remove it before each case.
                if valkey_rdb and valkey_rdb.exists():
                    try:
                        valkey_rdb.unlink()
                        print(f"  [INFO] Removed stale RDB cache: {valkey_rdb}")
                    except Exception as e:
                ...
                # valkey-1.1.0: pass NUM_CPU_CORES so the PTS launcher configures
                # both --io-threads (server MT) and --threads (benchmark MT).
                case_cmd = (
                    f'LOG_FILE={case_log} '
    ...
    def run_benchmark(self, num_threads):
        ...
        # PTS sanitizes identifiers (e.g. 1.1.0 -> 110), so we try to remove both forms
    ...
    def export_results(self):
        ...
        for num_threads in self.thread_list:
            ...
            if not result_dir.exists():
                ...
                if direct_results:
                    print("[INFO] PTS result not found; exporting from direct benchmark matrix results")
...
def main():
    parser = argparse.ArgumentParser(
        description="Valkey 1.1.0 Benchmark Runner (Multi-thread scaling: server io-threads + benchmark threads)",
