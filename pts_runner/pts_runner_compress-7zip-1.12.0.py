...
- TH_scaling: env:NUM_CPU_CORES - Uses -mmt=$NUM_CPU_CORES
"""
...
class Compress7zipRunner:
    def __init__(self, threads_arg=None):
        ...
        """
        self.benchmark = "compress-7zip-1.12.0"
    ...
    def patch_sha512_build(self):
        """
        Patch 7-Zip build to disable SHA512 optimizations that cause assembler errors.

        The vsha512rnds2 instruction is part of AVX512/SHA-512 extensions that may not
        be supported by older binutils. This method patches the Makefile to skip
        compiling Sha512Opt.c which contains these instructions.

        Returns:
            bool: True if patch successful, False otherwise
        """
        print(f"\n>>> Patching 7-Zip SHA512 optimization...")

        pts_home = Path.home() / ".phoronix-test-suite"
        install_dir = pts_home / "installed-tests" / "pts" / self.benchmark

        # Find the problematic source file
        sha512opt_files = list(install_dir.rglob("Sha512Opt.c"))
        if not sha512opt_files:
            print(f"  [WARN] Sha512Opt.c not found, cannot patch")
            return False

        # Find and patch the Makefile
        makefiles = list(install_dir.rglob("*gcc*.mak")) + list(install_dir.rglob("Makefile"))
        if not makefiles:
            print(f"  [WARN] No Makefile found, cannot patch")
            return False
        ...
        for makefile in makefiles:
            try:
                ...
                # Replace Sha512Opt.o compilation with empty target
                # Pattern: lines containing Sha512Opt.o compilation
                if 'Sha512Opt.o' in content:
                    print(f"  [INFO] Patching {makefile}")

                    # Comment out Sha512Opt.o compilation rules
                    content = re.sub(
                        r'^([^\n]*Sha512Opt\.o[^\n]*)$',
                        r'# PATCHED: \1  # Disabled due to assembler incompatibility',
                        content,
                        flags=re.MULTILINE
                    )

                    # Remove Sha512Opt.o from object file lists
                    content = re.sub(
                        r'\s*\$\(_O\)/Sha512Opt\.o\s*',
                        ' ',
                        content
                    )
                    content = re.sub(
                ...
                print(f"  [WARN] Failed to patch {makefile}: {e}")

        if not patched:
            print(f"  [WARN] No Makefile was successfully patched")
            return False

        print(f"  [OK] SHA512 optimization disabled")
        return True
    ...
    def install_benchmark(self):
        ...
        if result.returncode != 0:
            ...
            # Patch the build to disable problematic optimizations
            if self.patch_sha512_build():
                # Retry installation
                print(f"\n  [INFO] Retrying installation after patch...")
                result = subprocess.run(
                    ['bash', '-c', install_cmd],
                    text=True
                )
                if result.returncode != 0:
                    print(f"  [ERROR] Installation failed even after patching")
                    sys.exit(1)
            else:
                sys.exit(1)
    ...
    def run_benchmark(self, num_threads):
        ...
        batch_env = f'NUM_CPU_CORES={num_threads} BATCH_MODE=1 SKIP_ALL_PROMPTS=1 DISPLAY_COMPACT_RESULTS=1 TEST_RESULTS_NAME=compress-7zip-{num_threads}threads TEST_RESULTS_IDENTIFIER=compress-7zip-{num_threads}threads'
    ...
    def run(self):
        """Main execution flow."""
        print(f"{'='*80}")
        print(f"7-Zip Compression Benchmark Runner")
        print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(
        description='7-Zip Compression Benchmark Runner',
...
Examples:
  ...
  %(prog)s 16        # Run with 16 threads (capped at vCPU if exceeded)
    ...
    # Run benchmark
    runner = Compress7zipRunner(args.threads)
    success = runner.run()
