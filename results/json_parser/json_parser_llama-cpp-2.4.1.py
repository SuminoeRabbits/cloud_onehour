#!/usr/bin/env python3
"""llama-cpp-2.4.1 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で AI/llama-cpp-2.4.1 のみを抽出する。
"""
...
BENCHMARK_NAME = "llama-cpp-2.4.1"
...
def _collect_thread_payload(
    ...
    for _hash, entry in entries.items():
        ...
        result_systems = entry.get("results", {})
        if not isinstance(result_systems, dict):
            continue

        for _sys_id, sys_data in result_systems.items():
            ...
            if not isinstance(test_run_times, list):
                test_run_times = []
            ...
            key = f"{title} - {description} [{scale}]" if description else f"{title} [{scale}]"
            test_payload[key] = {
