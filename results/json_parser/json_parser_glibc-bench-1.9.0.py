#!/usr/bin/env python3
"""glibc-bench-1.9.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で System/glibc-bench-1.9.0 のみを抽出する。
"""
...
BENCHMARK_NAME = "glibc-bench-1.9.0"
...
def _find_machine_info_in_hierarchy(benchmark_dir: Path, search_root: Path) -> tuple:
