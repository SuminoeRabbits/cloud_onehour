#!/usr/bin/env python3
"""xmrig-1.3.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で Processor/xmrig-1.3.0 のみを抽出する。
"""
...
def _read_thread_freq(benchmark_dir: Path, thread_num: str, suffix: str) -> Dict[str, int]:
    """Read frequency file from either benchmark root or <N>-thread subdir."""
    filename = f"{thread_num}-thread_freq_{suffix}.txt"
    freqs = _read_freq_file(benchmark_dir / filename)
    if freqs:
        return freqs
    return _read_freq_file(benchmark_dir / f"{thread_num}-thread" / filename)
...
def _find_machine_info_in_hierarchy(benchmark_dir: Path, search_root: Path) -> tuple[str, str, str, Dict[str, Any]]:
    ...
    fallback_os_path = category_dir.parent
    fallback_machine_path = fallback_os_path.parent
    return fallback_machine_path.name, fallback_os_path.name, category_name, {}
...
def _collect_thread_payload(
    ...
    for entry in entries:
        ...
        # Example: "Xmrig - Variant: Monero - Hash Count: 1M [H/s]"
        key_base = f"{title} - {description}" if description else title
        key = f"{key_base} [{unit}]"
    ...
    # perf_stat 構築
    start_freq = _read_thread_freq(benchmark_dir, thread_num, "start")
    end_freq = _read_thread_freq(benchmark_dir, thread_num, "end")
    perf_stat = {}
    if start_freq:
