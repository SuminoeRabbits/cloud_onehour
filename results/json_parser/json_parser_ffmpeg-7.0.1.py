#!/usr/bin/env python3
"""ffmpeg-7.0.1 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で Multimedia/ffmpeg-7.0.1 のみを抽出する。
"""
...
def _load_thread_json(benchmark_dir: Path, thread_num: str) -> list[tuple[str, dict]]:
    ...
    for _hash, entry in data.get("results", {}).items():
        ...
        for _sys_id, sys_data in entry.get("results", {}).items():
            # Build test_key from title and description
            # Example: "FFmpeg" + "Encoder: libx264 - Scenario: Live" 
            # -> "FFmpeg - Encoder: libx264 - Scenario: Live"
            if description:
                test_key = f"{title} - {description}" if title else description
            else:
                test_key = title or "Unknown"
