#!/usr/bin/env python3
"""simdjson-2.1.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で Processor/simdjson-2.1.0 のみを抽出する。
"""
...
BENCHMARK_NAME = "simdjson-2.1.0"
TESTCATEGORY_HINT = "Processor"

# Regex for simdjson PTS output
# Example:
...
TEST_SECTION_RE = re.compile(
    r"Throughput Test:\s*(?P<test_name>[^:]+):\s*\n(?:\s*[\d.]+\s*\n)+\s*Average:\s*(?P<value>[\d.]+)\s*GB/s",
...
def _collect_thread_payload(
    ...
    for match in matches:
        ...
        test_payload[key] = {
            "description": f"simdjson Throughput: {test_name}",
            "values": value,
            "raw_values": [value],
            "unit": "GB/s",
            "time": 0.0,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "simdjson parser: cloud_onehour/results/<machinename> を入力に "
