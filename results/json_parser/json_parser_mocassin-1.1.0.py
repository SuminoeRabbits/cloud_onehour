#!/usr/bin/env python3
"""mocassin-1.1.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で FPU/mocassin-1.1.0 のみを抽出する。

パターン D（ビルド系）: <N>-thread.log の "Average: XXX Seconds" を解析し、
値そのものを time / test_run_times として使用する。
"""
...
# ---------------------------------------------------------------------------
# Benchmark-specific constants / regex
# ---------------------------------------------------------------------------

BENCHMARK_NAME = "mocassin-1.1.0"
TESTCATEGORY_HINT = "FPU"

# PTS log output example:
#   Time to Complete:
#       1234.567
#
#   Average: 1234.567 Seconds
...
def _find_machine_info_in_hierarchy(
    ...
    while current != search_root.parent and current != current.parent:
        ...
        # Valid machine: CSP must be present and not "unknown"
...
# ---------------------------------------------------------------------------
# Benchmark-specific extraction (Pattern D: value = execution time)
# ---------------------------------------------------------------------------
...
def _collect_thread_payload(
    ...
    Pattern D: the measured value IS the total execution time in seconds,
    so time == values and test_run_times == [values].
    """
    ...
    test_payload: Dict[str, Any] = {
        "mocassin [Seconds]": {
            "description": "Mocassin Photoionization",
            "values": value,
            "raw_values": [value],
            "unit": "Seconds",
            "time": value,
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "mocassin parser: cloud_onehour/results/<machinename> を入力に "
