#!/usr/bin/env python3
"""srsran-2.5.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で Telecom/srsran-2.5.0 のみを抽出する。

特記: このベンチマークはすべてのデータを <N>-thread.json から取得する
（パターン C の JSON-only 変種）。

Tests (4種):
  - PDSCH Processor Benchmark, Throughput Total   (Mbps, HIB)
  - PUSCH Processor Benchmark, Throughput Total   (Mbps, HIB)
  - PDSCH Processor Benchmark, Throughput Thread  (Mbps, HIB)
  - PUSCH Processor Benchmark, Throughput Thread  (Mbps, HIB)
"""
...
BENCHMARK_NAME = "srsran-2.5.0"
TESTCATEGORY_HINT = "Telecom"
...
def _find_machine_info_in_hierarchy(
    ...
    # Fallback: no valid machinename found in hierarchy
    _os_dir = category_dir.parent
    _machine_dir = _os_dir.parent
    return _machine_dir.name, _os_dir.name, category_name, {}
...
def _collect_thread_payload(
    ...
    """Build the `<thread>` node from <N>-thread.json.

    All data (value, description, test_run_times) comes from the JSON file.
    """
    ...
    for _hash, entry in results.items():
        title = entry.get("title", "srsRAN Project")
        app_version = entry.get("app_version", "")
        description = entry.get("description", "")
        scale = entry.get("scale", "Mbps")
        ...
        for _sys_id, sys_data in entry.get("results", {}).items():
            ...
            # Key: "<title> <app_version> - <description> [{scale}]"
            # Include [{scale}] to avoid key collision across tests with same title/description.
            if description:
                key = f"{title} {app_version} - {description} [{scale}]"
            else:
                key = f"{title} {app_version} [{scale}]"
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "srsran-2.5.0 parser: cloud_onehour/results/<machinename> を入力に "
