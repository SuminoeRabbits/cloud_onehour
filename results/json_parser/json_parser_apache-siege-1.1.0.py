#!/usr/bin/env python3
"""apache-siege-1.1.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で Network/apache-siege-1.1.0 のみを抽出する。
"""
...
BENCHMARK_NAME = "apache-siege-1.1.0"
...
def _safe_get_machine_info(machinename: str) -> Dict[str, Any]:
    """Call get_machine_info() without polluting stdout."""
    with contextlib.redirect_stdout(io.StringIO()):
        return get_machine_info(machinename)
...
def _find_machine_info_in_hierarchy(
    ...
    while current != search_root.parent and current != current.parent:
        machine_info = _safe_get_machine_info(current.name)
    ...
    fallback_parent_dir = fallback_os_dir.parent
    return fallback_parent_dir.name, fallback_os_dir.name, category_name, {}


def _load_thread_json(benchmark_dir: Path, thread_num: str) -> list[tuple[str, Dict[str, Any]]]:
    """Load <N>-thread.json and return parsed test entries."""
    json_file = benchmark_dir / f"{thread_num}-thread.json"
    if not json_file.exists():
        return []

    try:
        data = json.loads(_strip_ansi(json_file.read_text(encoding="utf-8", errors="replace")))
    ...
    for _hash, entry in data.get("results", {}).items():
        ...
        scale = str(entry.get("scale", "")).strip()
        ...
        for _sys_id, sys_data in entry.get("results", {}).items():
            ...
            raw_values = sys_data.get("raw_values") or [value]
            test_run_times = sys_data.get("test_run_times") or []
            test_key = f"{title} - {description}" if description else title

            key_with_unit = f"{test_key} [{scale}]"
            entries.append(
                (
                    key_with_unit,
                    {
...
def _collect_thread_payload(
    ...
    for test_key, test_info in entries:
        ...
        raw_values = test_info["raw_values"]
        test_run_times = test_info["test_run_times"]
        scale = test_info["scale"]
        description = test_info["description"]

        time_val = statistics.median(test_run_times) if test_run_times else 0.0
...
def _build_full_payload(search_root: Path) -> Dict[str, Any]:
    """Search for benchmark directories recursively and build JSON payload."""
    ...
    for benchmark_dir in sorted(search_root.glob(f"**/{BENCHMARK_NAME}")):
        ...
        if not machine_info:
            machine_info = _safe_get_machine_info(machinename)
...
def main() -> None:
    ...
    payload = _build_full_payload(args.search_root.resolve())
