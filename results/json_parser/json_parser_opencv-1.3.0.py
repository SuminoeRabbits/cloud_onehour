#!/usr/bin/env python3
"""opencv-1.3.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で FPU/opencv-1.3.0 のみを抽出する。
...
ログ形式:
    pts/opencv-1.3.0 [Test: DNN - Deep Neural Network]
    ...
    Average: 175033 ms
    Samples: 1
"""
...
BENCHMARK_NAME    = "opencv-1.3.0"
TESTCATEGORY_HINT = "FPU"

ANSI_ESCAPE_RE = re.compile(r"\x1B[@-_][0-?]*[ -/]*[@-~]")

# Match one test block: from "[Test: NAME]" header to "Average: VALUE ms"
# Captures: (test_name, average_ms)
TEST_BLOCK_RE = re.compile(
    r"pts/opencv-\S+\s+\[Test:\s*([^\]]+)\]"   # [Test: DNN - Deep Neural Network]
    r".*?"                                        # skip intervening lines
    r"Average:\s*([\d.]+)\s+ms",                 # Average: 175033 ms
...
def _find_machine_info_in_hierarchy(
    ...
    return fallback_os_dir.parent.name, fallback_os_dir.name, category_name, {}
...
def _collect_thread_payload(
    ...
    """Extract per-test results from <N>-thread.log."""
    ...
    for match in TEST_BLOCK_RE.finditer(content):
        test_name = match.group(1).strip()   # e.g. "DNN - Deep Neural Network"
        value_ms  = float(match.group(2))    # e.g. 175033.0

        # Convert ms → seconds for cost calculation
        time_sec = value_ms / 1000.0
        cost     = round(cost_hour * time_sec / 3600, 6) if time_sec > 0 else 0.0

        # Key: "OpenCV - <TestName> [ms]"
        key = f"OpenCV - {test_name} [ms]"
