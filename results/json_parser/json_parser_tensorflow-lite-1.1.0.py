...
データ構造（抜粋）を生成し、TensorFlow Lite 1.1.0 のみを対象に出力する。
"""
...
def _extract_test_entries(thread_json: Path) -> List[Dict[str, Any]]:
	...
	for test_block in data.get("results", {}).values():
		...
		for system_data in test_block.get("results", {}).values():
			...
			details = system_data.get("details") or {}
			error_message = details.get("error") if isinstance(details, dict) else None
...
def _build_test_node(entry: Dict[str, Any], cost_hour: float) -> Dict[str, Any]:
	"""Create README 準拠の test_name ノード。"""
...
def _build_thread_node(entries: List[Dict[str, Any]], cost_hour: float) -> Dict[str, Any]:
	"""thread ノード全体 (perf_stat + test_name) を構築する。"""
...
def _parse_tflite_benchmark(benchmark_dir: Path, cost_hour: float) -> Dict[str, Any]:
	"""README 形式の benchmark ノード (`thread` 階層) を返す。"""
...
def _build_machine_payload(machine_dir: Path) -> Dict[str, Any]:
	"""`<machinename>` ディレクトリ以下を TensorFlow Lite だけ抜き出して構造化する。"""
	...
	for os_dir in sorted([p for p in machine_dir.iterdir() if p.is_dir()]):
		...
		for testcategory_dir in sorted([p for p in os_dir.iterdir() if p.is_dir()]):
			...
			if not benchmark_dir.is_dir():
				continue
			thread_nodes = _parse_tflite_benchmark(benchmark_dir, cost_hour)
			if not thread_nodes:
				continue
...
def main() -> None:
	parser = argparse.ArgumentParser(
		description="cloud_onehour/results/<machinename> を入力に TensorFlow Lite 1.1.0 構造を JSON で出力する"
	)
	parser.add_argument(
		"--dir",
		"-d",
