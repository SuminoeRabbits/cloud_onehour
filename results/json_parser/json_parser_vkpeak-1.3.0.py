#!/usr/bin/env python3
"""vkpeak-1.3.0 専用 JSON パーサー。

`cloud_onehour/results/<machinename>` を入力に README_results.md と同じ
データ構造（抜粋）で FPU/vkpeak-1.3.0 のみを抽出する。
"""
...
def _find_machine_info_in_hierarchy(benchmark_dir: Path, search_root: Path) -> tuple[str, str, str, Dict[str, Any]]:
    ...
    fallback_machine_name = fallback_os_dir.parent.name
    return fallback_machine_name, fallback_os_dir.name, category_name, {}


def _load_thread_json(benchmark_dir: Path, thread_num: str) -> list[dict[str, Any]]:
...
def _collect_thread_payload(
    ...
    for entry in entries:
        ...
        if isinstance(test_run_times, list) and test_run_times:
